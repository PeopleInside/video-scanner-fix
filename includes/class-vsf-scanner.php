...
class Video_Scanner_Fix_Scanner {
    ...
    public function scan_post($post) {
        ...
        // Clean up old log entries for this post that are no longer present in post content
        if ($post->ID > 0) {
            global $wpdb;
            $table = Video_Scanner_Fix_Logger::get_table_name();
            $old_logs = $wpdb->get_results($wpdb->prepare("SELECT id, video_url FROM {$table} WHERE post_id = %d", $post->ID), ARRAY_A);

            if (!empty($old_logs)) {
                // Presence check across ALL platforms
                $all_platform_keys = array_keys($this->get_patterns());
                ...
                foreach ($old_logs as $old_log) {
                    ...
                    if (!in_array($url, $all_found_urls, true)) {
                        // Video URL is no longer in post content/meta -> remove log entry
